...
function out_code(){		
	...
	if(trimString(textCode.value).length>0 && trimString(textCode.value).length<12){
		stockinAction.findStockinByCode(textCode.value,callback);
	}else{
		show_message("msg_code","0","进货单编号不能为空且不能超过11位长度！");
		codeChk = false;
	}
}
function callback(ret){
	if(ret > 0){
		show_message("msg_code","0","输入的进货单编号["+textCode.value+"]重复请重新输入！");
		codeChk = false;
	}else{
...
function formSubmit(){
	...
	if(codeChk && chkRetMerchandise && chkRetAmount && chkRetStockindate && chkRetEmployee && chkRetClient){
		chk = true;
	}
	return chk;
}
...
<body>
<p></p>
<p><font style="font-size: 10pt;">进货管理->进货单->新增</font></p>
<s:form action="stockinAction" method="post" theme="simple"
	...
	<table border="0" cellpadding="1" cellspacing="1" width="95%">
		<tr>
			...
			<td width="20%"><s:textfield name="stockin.code" id="code"
				onfocus="show_message('msg_code','2','请输入进货单编号！');"
				onblur="out_code()" /></td>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="95%">
		<tr>
			<td width="10%"><s:submit value="保存" method="saveStockin" /></td>
